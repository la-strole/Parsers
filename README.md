...
3. Firefox версии выше 48 (для работы с сайтом московской фондовой биржи);
4. Подключение к Интернет (для скачивания библиотек python).
...
### Запуск:
1. Запуск всех парсеров по очереди:
  1. Запустить `start_parsers.sh`.<br/>
   ...
   2. логи пишутся в `log.txt` в рабочем каталоге.
2. Запуск парсеров по отдельности:
  1. Перейти в созданную virtual environment (`$ source parsers_venv/bin/activate`);
  2. Запустить нужный парсер с обязательным аргументом командной строки `-path` (`(parsers_venv)$ python <name_of_parser>.py -path path/to/working/dir/`);
  3. Парсер сохранит `*csv` по заданному `-path`пути;
  4. **Внимание:** При запуске `moex_bonds.py` необходимо обратить внимание на то, что для корректной работы selenium необходимо, чтобы geckodriver находился в каталоге `-path` либо путь к нему находился в PATH ОС.
