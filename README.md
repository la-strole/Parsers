...
Все парсеры можно использовать независимо от libreoffice calc файла. Планирую использовать авито для мониторинга изменения цены недвижимости в Пскове.

### Текущее использование следующее:
запускаем bash скрипт, который активирует `venv` и запускает `main.py`, который, отрабатывая, создает набор файлов `*csv`, которые, в свою очередь подгружаются как external data в **существующий файл libreoffice calc**, где есть наборы формул, графиков итп для аналитики и планирования.

### Собираем следующие данные:
1. `avito_plat.py` (https://avito.ru) - стомость однокомнатных квартир по адресу г. Псков, Крестки 3А;
...
6. `main.py` - собирает все вместе.

### Создание окружения:
1. распоковать `venv.zip`;
2. в `start_parsers.sh` отредактировать `path_line` на путь к каталогу с `venv`.

### Запуск:
1. запустить `start_parsers.sh`;
2. логи пишутся в `log.txt` в том же каталоге.

### Результат:
По умолчанию в каталоге с `venv` (путь можно изменить в переменной `path_to_csv` в `main.py`) создаются файлы:

1. `for_open_office0.csv`
*[стоимость_портфеля, timestamp]*
...
2. `for_open_office1.csv`
*[количество_квартир, количество_цен]*
*[Продана_ли(True/False), идентификатор(кол-во комнат площадь, этаж), стоимость1, стоимость2,...]*
...
3. `for_open_office2.csv`
*[текущая стоимость в проценах от номинала облигации XS0893212398]*
*[текущая стоимость в проценах от номинала облигации XS0088543193]*
4. `for_open_office3.csv`
*[лучшая цена покупки грамма золота, название банка, timestamp]*
5. `for_open_office4.csv`
*[стоимость монеты Георгий победоносей 100 в банке Уралсиб]*
6. `for_open_office5.csv`
*[лучшая цена покупки USD, название банка, timestamp]*
*[лучшая цена покупки EUR, название банка, timestamp]*
*[лучшая цена продажи USD, название банка, timestamp]*
*[лучшая цена продажи EUR, название банка, timestamp]*
*[курс ЦБРФ USD, CBRF, timestamp]*
*[курс ЦБРФ EUR, CBRF, timestamp]*

### Использованы библиотеки:
1. requests;
2. BeautifulSoup;
3. selenium - для парсига мосбиржи (https://moex.com) (необходимо нажать кнопку согласия, данные формируюся JS и обновляются раз в 5 секунд).
